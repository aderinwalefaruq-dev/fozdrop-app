...
export default function CheckoutScreen() {
  ...
  const { plates, vendors, subtotal, clearCart, updateItemQuantity, removeItemFromPlate } = useCart();
  ...
  const handlePlaceOrder = async () => {
    ...
    const vendorGroups = vendors.map((v) => {
      const vendorPlates = plates.filter((p) => p.vendor.id === v.id);
  ...
  if (plates.length === 0 && !showSuccess) {
    return (
